...
- Запускает текущий EasyOCR, получает word-level bounding boxes + conf.
- Строит набор GT-токенов из двух источников:
  - `inputs/<pdf>.txt` — plain-text транскрипция страниц
  - `expected/<pdf>.json` — структурированные эталоны (юрлица, ИНН/КПП/
    ОГРН, адреса, банки, БИК, номера документов, даты). JSON добавляет
    доменный словарь, которого нет или мало в .txt.
- Для каждого box'а:
  - фильтрует по confidence (`--min-conf 0.2` по умолчанию);
  - сверяет предсказание с GT через Левенштейна (budget 1 для коротких,
    2 для длинных слов);
  - **по умолчанию (строгий режим)** — выбрасывает крон, если в GT
    ничего похожего нет (EasyOCR кривыми стоковыми весами на наших
    сканах даёт CER 174%, поэтому несверенные лейблы — в основном мусор);
  - если есть точное совпадение — keeps as is; фуззи-совпадение —
    подставляет GT как label; сохраняет crop как `img_NNNNNNNN.jpg`.
...
- `--keep-unverified` — старое поведение: сохраняет crops даже если
  их label не нашёл GT-совпадения (больше данных, но много шума).
- `--min-conf 0.3` — более строгий фильтр по уверенности EasyOCR.
...
- `inputs/*.txt` + `expected/*.json` — page-level / структурированная
  эталонка, не per-word bbox-annotations. Auto-correction покрывает
  только тот word, что EasyOCR уже извлёк «близко» к GT-токену. В
  строгом режиме (default) это значит, что сильно искажённые / не
  распознанные слова **выбрасываются**, а не остаются с шатким лейблом.
  Для handwriting и мелкого/сломанного текста это означает недобор
  данных — компромисс в пользу качества сигнала.
