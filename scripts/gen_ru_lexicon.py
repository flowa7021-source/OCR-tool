...
Стратегия (апрель 2026, v2): берём ВСЕ формы 5-12 chars из
pymorphy3 OpenCorpora-словаря. Это ~2M уникальных форм, покрывает
подавляющее большинство словарного запаса бизнес-текстов на
русском. Файл ~41 MB, загружается за ~800ms на первый call
...
Почему 5-12 chars:
  ...
  * 5-12 chars — sweet spot: покрывает «организация»/«организации»
    (11 chars), «предприятие» (11), «документация» (12). Более
    узкое 5-10 оставляло за бортом ключевые business-слова, и
    rapidfuzz возвращал близкие но неправильные substring'и
    («организма» вместо «организация»);
  * > 12 chars — редко встречаются в OCR-ящихся полях ТН
    (названия организаций типа «Моспроект-3» больше, но там другое
    исправление через catalog).

Runtime pymorphy3 ТРЕБУЕТСЯ для proper-noun guard в
``morphology_validator.py`` — без неё fuzzy_corrector fallback'ит
на старое поведение и может подставлять функциональные слова вместо
имён собственных. Для сборки без pymorphy3 держите словарь в 5-10.
