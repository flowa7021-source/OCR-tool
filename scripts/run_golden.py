...
def _strip_ocr_suffix(stem: str) -> tuple[str, str | None]:
...
def _match_expected(pdf: Path) -> Path | None:
...
def _date_iso_to_ru(s: str | None) -> str | None:
    if not s:
        return None
...
def _first_tn(doc: dict) -> dict | None:
...
def _ru_date_to_iso(s: str) -> str | None:
    if not s:
        return None
...
def _party(tn: dict, role: str) -> dict | None:
...
def expected_fields(expected: dict) -> dict[str, Any]:
    ...
    loading = tn.get("loading", {}) or {}
    out["reception"] = loading.get("infrastructure_owner")
    return out
