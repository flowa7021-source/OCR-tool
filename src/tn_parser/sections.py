...
_FORM_COMMENT_ANCHORS: list[tuple[str, str]] = [
    ...
    # «перевозчик» / «перевозщик» / «перевок» / «псрсвозч» / «ПЕРЕВОЗКИ»
    # — все принимаем, ключевой stem «возч» / «возк» / «возщ».
    ("carrier", r"п[еc][рп]?[еc]во[зc][чщк]|перевок"),
...
def _find_form_comment_anchors(text: str) -> list[tuple[int, str]]:
    ...
    # Маркер «explanatory comment стандартной формы ТН» — должен
    # быть НЕПОСРЕДСТВЕННО перед anchor'ом, не где-то в окне.
    # «идентифицировать <Роль>» — самый сильный сигнал; «позволяющие
    # идентифицировать» — ещё сильнее. OCR может мангнуть «ф» → «й»,
    # «ц» → «н»: «идентийнииривать», «зволяющие». Допускаем.
    context_re = re.compile(
        r"(?:идент[ийн][фйн]\w*|"
        r"зволя[юя]щ\w*|позволя\w*)",
        re.IGNORECASE,
    )
